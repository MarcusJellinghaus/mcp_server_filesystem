...
skip_pr_wait_msg = None
if wait_for_pr:
    effective_pr_timeout = pr_timeout if pr_timeout > 0 else _DEFAULT_PR_TIMEOUT
    if branch and not await to_thread(remote_branch_exists, project_dir, branch):
        skip_pr_wait_msg = "Push branch to remote before waiting for PR"
    elif branch:
        await _wait_for_pr(project_dir, branch, effective_pr_timeout)

if ci_timeout > 0 and branch:
    await _wait_for_ci(project_dir, branch, ci_timeout)

report = await to_thread(collect_branch_status, project_dir, max_log_lines)

if skip_pr_wait_msg:
    report = replace(report, recommendations=[skip_pr_wait_msg, *report.recommendations])

return report.format_for_llm()
```

Note: when `branch is None` (not in a git repo), skip both waits — `collect_branch_status` will return the empty report.
...
1. **Defaults** (`wait_for_pr=False`, both timeouts `0`): neither helper is called; `collect_branch_status` is called once; output equals `format_for_llm()` of the fixture report.
2. **`ci_timeout=30`**: `_wait_for_ci` called with `(project_dir, branch, 30)`; `_wait_for_pr` not called.
3. **`wait_for_pr=True`, branch has remote tracking**: `_wait_for_pr` called with `_DEFAULT_PR_TIMEOUT=600`; recommendation NOT prepended.
4. **`wait_for_pr=True`, `pr_timeout=120`, branch has remote tracking**: `_wait_for_pr` called with `120`.
5. **`wait_for_pr=True`, no remote tracking branch**: `_wait_for_pr` NOT called; output starts with the recommendation `"Push branch to remote before waiting for PR"` (verify it's in `report.recommendations` of the formatted output).
6. **Order check**: when both `wait_for_pr=True` and `ci_timeout>0` and remote tracking exists, `_wait_for_pr` is awaited before `_wait_for_ci`, and `collect_branch_status` runs after both. Use a list captured by side-effects to assert order.
7. **No branch** (`get_current_branch_name` returns `None`): neither helper called; report still produced.
