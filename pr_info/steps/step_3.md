...
4. if not patch_portion:
       ...
       optionally prepend the "# Compact diff: ..." header (line counts from patch_portion only)
...
1. `test_show_compact_preserves_commit_header` — call `git_show(project_dir, args=["HEAD"])`; assert the result contains `"Author:"` and `"Date:"` (commit header) **and** `"diff --git"` (patch).
...
9. `test_show_compact_stat_and_patch_preserves_prefix` — call with `args=["HEAD", "--stat", "-p"]`; assert both `"|"` (stat) and `"diff --git"` are present.
