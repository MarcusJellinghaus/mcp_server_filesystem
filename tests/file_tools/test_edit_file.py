...
class TestEditFileUtils(unittest.TestCase):
    ...
    def test_get_indentation(self):
        self.assertEqual(get_indentation("    indented line"), "    ")
        self.assertEqual(get_indentation("\t\tindented line"), "\t\t")
        self.assertEqual(get_indentation("no indentation"), "")
    ...
    def test_find_match_exact(self):
        content = "line1\nline2\nline3\nline4"
        pattern = "line2\nline3"
        result = find_match(content, pattern)
    ...
    def test_find_match_fuzzy(self):
        ...
        pattern = "line2\nline3"
        result = find_match(content, pattern, True, 0.7)
        self.assertTrue(result.matched)
        self.assertGreater(result.confidence, 0.7)

    def test_find_match_threshold(self):
        # Test that the threshold parameter works
        content = "line1\nline2 with extra\nline3\nline4"
        pattern = "line2\nline3"
        # With high threshold, should not match
        result_high = find_match(content, pattern, True, 0.95)
        self.assertFalse(result_high.matched)
        # With lower threshold, should match
        result_low = find_match(content, pattern, True, 0.7)
        self.assertTrue(result_low.matched)
